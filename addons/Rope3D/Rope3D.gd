...
var joint_a: Joint3D
var joint_b: Joint3D
var anchor_a: Marker3D
var anchor_b: Marker3D
...
func create_rope():
	...
	mesh_instance.translate(-get_global_transform().origin)
...
func get_parent_physic_body(node: Node):
	while node and !node is PhysicsBody3D:
...
func create_segment(global_position: Vector3, global_direction: Vector3):
	...
	if collision_monitor:
		...
		segment.rope = self
		segment.body_entered.connect(segment._on_body_entered())
		segment.rope_body_entered.connect(_on_rope_body_entered)
...
func create_joint(local_position: Vector3, direction: Vector3, a: Node, b: Node):
	...
	joint.translate(local_position)
...
func update_rope_geometry():
	var v := 0
	var cam_trx := get_viewport().get_camera_3d().get_global_transform()#get_viewport().get_camera().get_global_transform()
	...
	mesh.clear_surfaces()
	#mesh.surface_remove(0) # this function doesn't exists in Godot 4.1 yet
